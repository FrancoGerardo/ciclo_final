...
<body>
        ...
        <table class="table table-striped  shadow-lg mt-4">
            ...
            @foreach ($carrito as $carritos)
                @if ($carritos->id_user == $userActual->id)
                    @foreach ($detalleCarrito as $detalleActual)
                        @foreach ($producto as $productos)
                            @if ($detalleActual->producto_id == $productos->id)
                                <tbody>
                                    <div class="row border">
                                        <tr>
                                            <td class="text-center">
                                                <a><img width="70" height="50" alt="poster_1_up"
                                                        class="shop_thumbnail" src="{{ $productos->foto }}"></a>
                                            </td>
                                                    ...
                                                    @if ($productos->id_inventario == $inventarios->id)
                                                        <div id="contador" class="input-counter">
                                                            <input
                                                                ...
                                                                value="{{ $detalleActual->cantidad }}" min="1"
                                                                max="{{ $inventarios->cant_disponible }}" disabled>
                                            ...
                                            <td>
                                                <form action = "{{ route('carritos.destroy', $detalleActual->producto_id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"><a title="Remove this item"
                                                            class="remove">X</a></button>
                                                </form>
    ...
    <center>
        ...
        <button class="btn " style="background-color:#111010;color:#ffff" href=""
            onclick="{{ route('frontal.notaventa_index') }}">
            <i class="fa fa-shopping-cart"></i> Comprar</button> 
                        ...
                        <div class="footer">
                            ...
                            <ul class="footer-links">
                                ...
                                <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
                                    ...
                                    <img alt="" border="0"
                                        src="https://www.paypal.com/en_BO/i/scr/pixel.gif" width="1"
                                        height="1" />
                                </form>
                            </ul>
                        </div>
